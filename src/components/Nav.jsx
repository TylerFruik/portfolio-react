import React from 'react';
import { Link } from 'react-router-dom';
import CoherenceToggle from '../coherence/CoherenceToggle';

const Nav = () => {
  return (
    <header>
      {/* v1/v2 view switch (2026-08-28) -- Tyler: "I want the v1/v2 toggle button to be up in the
          corner next to Tyler Fruik." v1's own "Tyler Fruik" title sits lower in this banner
          (bottom:10px, see styles.css .banner h1), so this rides the header's top-left corner
          above it rather than overlapping the title itself. */}
      <div className="cx-v1-toggle-corner"><CoherenceToggle /></div>
      <div className="wave">
        <svg data-name="Layer 1" xmlns="http://www.w3.org/2000/svg" viewBox="0 50 200 10" preserveAspectRatio="none">
          <path d="M321.39,56.44c58-10.79,114.16-30.13,172-41.86,82.39-16.72,168.19-17.73,250.45-.39C823.78,31,906.67,72,985.66,92.83c70.05,18.48,146.53,26.09,214.34,3V0H0V27.35A600.21,600.21,0,0,0,321.39,56.44Z" className="shape-fill"></path>
        </svg>
      </div>
      <div className="wave2">
        <svg data-name="Layer 2" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 120" preserveAspectRatio="none">
          <path d="M321.39,56.44c58-10.79,114.16-30.13,172-41.86,82.39-16.72,168.19-17.73,250.45-.39C823.78,31,906.67,72,985.66,92.83c70.05,18.48,146.53,26.09,214.34,3V0H0V27.35A600.21,600.21,0,0,0,321.39,56.44Z" className="shape-fill"></path>
        </svg>
      </div>
      <div className="wave3">
        <svg data-name="Layer 3" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 800 80" preserveAspectRatio="none">
          <path d="M321.39,56.44c58-10.79,114.16-30.13,172-41.86,82.39-16.72,168.19-17.73,250.45-.39C823.78,31,906.67,72,985.66,92.83c70.05,18.48,146.53,26.09,214.34,3V0H0V27.35A600.21,600.21,0,0,0,321.39,56.44Z" className="shape-fill"></path>
        </svg>
      </div>
      <Link to='/'>
        <h1>Tyler Fruik</h1>
      </Link>
      <nav className='navbar'>
        <Link to='/'>About Me</Link>
        <Link to='/portfolio'>Portfolio</Link>
        <Link to='/resume'>Resume</Link>
      </nav>
    </header>
  )
}

export default Nav;
